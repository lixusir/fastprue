...
if (!function_exists('write_log')) {
    function write_log($msg, $filename = '', $clear = false, $dir = 'record')
    {
        $dir = __DIR__ . '/../logs/' . $dir . '/' . date('Ym') . '/' . date('d');
 ...
 * @param $str
 * @param int $length
 * @param bool $isForce 是否强制需要小数点后几位
 * @param bool $isCeil 是否需要进一取整
 * @return float|int
 */
function assetsIntercept($str,$length=2,$isForce=false,$isCeil=false){
    $num = pow(10,$length);
    if($isCeil){
        $result = ceil(bcmul($str,$num))/$num;
    }else{
        $result = floor(bcmul($str,$num))/$num;
    }
    if($isForce){
        return sprintf("%.".$length."f",$result);
    }
    return $result;
}